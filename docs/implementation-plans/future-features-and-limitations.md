...
| **MaxDiff message testing not supported** — Projects using MaxDiff methodology (message testing with Anchored Probability Index scores) have unique data structures that require specialized handling: message-to-variable matching, alternate message grouping, and careful treatment to avoid splitting API variables across tables. Full implementation plan exists (`maxdiff-message-testing-support.md`) but is deferred to allow focus on core functionality. Current pipeline may attempt to process MaxDiff data but results will likely be incorrect (split variables, truncated labels). | Medium | Dedicated implementation sprint after core product stabilizes. Estimated 5-8 days for full feature. UI currently shows "Coming Soon" for MaxDiff project type. | Documented, UI disabled |
