...
export async function GET(
  ...
  try {
    ...
    const convexFeedback = runResult?.feedback as unknown[] | undefined;
    ...
    if (convexFeedback && convexFeedback.length > 0) {
      return NextResponse.json({
        ...
        summary: {
          ...
          lastSubmittedAt: (convexFeedback[convexFeedback.length - 1] as Record<string, unknown>)?.createdAt || '',
          lastRating: (convexFeedback[convexFeedback.length - 1] as Record<string, unknown>)?.rating || 0,
        },
      });
    }
