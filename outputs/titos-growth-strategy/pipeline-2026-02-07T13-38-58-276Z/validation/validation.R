...
# Generated: 2026-02-07T13:49:35.004Z
# Tables to validate: 63
...
validation_results[["s5"]] <- tryCatch({
  ...
  # Row 4: S5 == 3,4
  if (!("S5" %in% names(data))) stop("Variable 'S5' not found")
  test_val <- sum(as.numeric(data[["S5"]]) %in% c(3, 4), na.rm = TRUE)
  # Row 5: S5 == 3
  if (!("S5" %in% names(data))) stop("Variable 'S5' not found")
...
validation_results[["s6a"]] <- tryCatch({
  ...
  # Row 1: NET - Any country/region listed (NET)
...
validation_results[["s8_binned"]] <- tryCatch({
  ...
  # Row 1: Category header - Number of alcoholic drinks in the past week — distribution (binned) (skip validation)
  # Row 2: S8 == 1
  if (!("S8" %in% names(data))) stop("Variable 'S8' not found")
  test_val <- sum(as.numeric(data[["S8"]]) == 1, na.rm = TRUE)
  # Row 3: S8 == 2-4
  if (!("S8" %in% names(data))) stop("Variable 'S8' not found")
  test_val <- sum(as.numeric(data[["S8"]]) >= 2 & as.numeric(data[["S8"]]) <= 4, na.rm = TRUE)
  # Row 4: S8 == 5-9
  if (!("S8" %in% names(data))) stop("Variable 'S8' not found")
  test_val <- sum(as.numeric(data[["S8"]]) >= 5 & as.numeric(data[["S8"]]) <= 9, na.rm = TRUE)
  # Row 5: S8 == 10-19
  if (!("S8" %in% names(data))) stop("Variable 'S8' not found")
  test_val <- sum(as.numeric(data[["S8"]]) >= 10 & as.numeric(data[["S8"]]) <= 19, na.rm = TRUE)
  # Row 6: S8 == 20-50
  if (!("S8" %in% names(data))) stop("Variable 'S8' not found")
  test_val <- sum(as.numeric(data[["S8"]]) >= 20 & as.numeric(data[["S8"]]) <= 50, na.rm = TRUE)
...
validation_results[["s10b"]] <- tryCatch({
  ...
  # Row 20: Category header - Indulgence (skip validation)
  # Row 21: S10b == 21
  if (!("S10b" %in% names(data))) stop("Variable 'S10b' not found")
  ...
  # Row 29: Category header - Performance (skip validation)
  # Row 30: S10b == 31
  if (!("S10b" %in% names(data))) stop("Variable 'S10b' not found")
  ...
  # Row 38: Category header - Other (open-ends) (skip validation)
  # Row 39: S10b == 100
  if (!("S10b" %in% names(data))) stop("Variable 'S10b' not found")
...
validation_results[["a4"]] <- tryCatch({
  ...
  # Row 7: NET - Hard Seltzer / RTD (NET)
  if (!("A4" %in% names(stacked_loop_1))) stop("NET component variable 'A4' not found")
  if (!("A4" %in% names(stacked_loop_1))) stop("NET component variable 'A4' not found")
  # Row 8: A4 == 7
  if (!("A4" %in% names(stacked_loop_1))) stop("Variable 'A4' not found")
...
validation_results[["a10_dist_bins"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a10_dist_bins", rowCount = 5)
...
validation_results[["a22_casamigos_column_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_casamigos_column_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_casamigos_column_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_casamigos_column_", "-", if(validation_results[["a22_casamigos_column_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_casamigos_column_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_casamigos_column_c5_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_casamigos_column_c5_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_casamigos_column_c5_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_casamigos_column_c5_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_casamigos_column_c5_", "-", if(validation_results[["a22_casamigos_column_c5_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_casamigos_column_c5_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_don_julio_column_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_don_julio_column_"]] <- tryCatch({

  # Row 1: Category header - Attributes (asked for each brand) (skip validation)
  # Row 2: A22r1c6 == 1
  ...
  list(success = TRUE, tableId = "a22_don_julio_column_", rowCount = 11)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_don_julio_column_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_don_julio_column_", "-", if(validation_results[["a22_don_julio_column_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_don_julio_column_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_don_julio_column_c6_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_don_julio_column_c6_"]] <- tryCatch({

  # Row 1: NET - Any attribute associated with Don Julio (NET)
  ...
  list(success = TRUE, tableId = "a22_don_julio_column_c6_", rowCount = 11)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_don_julio_column_c6_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_don_julio_column_c6_", "-", if(validation_results[["a22_don_julio_column_c6_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_don_julio_column_c6_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_grey_goose_column_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_grey_goose_column_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_grey_goose_column_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_grey_goose_column_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_grey_goose_column_", "-", if(validation_results[["a22_grey_goose_column_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_grey_goose_column_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_grey_goose_column_c2_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_grey_goose_column_c2_"]] <- tryCatch({

  # Row 1: Category header - Grey Goose (skip validation)
  # Row 2: A22r1c2 == 1
  if (!("A22r1c2" %in% names(data))) stop("Variable 'A22r1c2' not found")
  test_val <- sum(as.numeric(data[["A22r1c2"]]) == 1, na.rm = TRUE)
  # Row 3: A22r2c2 == 1
  ...
  list(success = TRUE, tableId = "a22_grey_goose_column_c2_", rowCount = 11)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_grey_goose_column_c2_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_grey_goose_column_c2_", "-", if(validation_results[["a22_grey_goose_column_c2_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_grey_goose_column_c2_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_ketel_one_column_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_ketel_one_column_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_ketel_one_column_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_ketel_one_column_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_ketel_one_column_", "-", if(validation_results[["a22_ketel_one_column_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_ketel_one_column_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_ketel_one_column_c3_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_ketel_one_column_c3_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_ketel_one_column_c3_", rowCount = 11)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_ketel_one_column_c3_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_ketel_one_column_c3_", "-", if(validation_results[["a22_ketel_one_column_c3_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_ketel_one_column_c3_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_maker_s_mark_column_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_maker_s_mark_column_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_maker_s_mark_column_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_maker_s_mark_column_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_maker_s_mark_column_", "-", if(validation_results[["a22_maker_s_mark_column_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_maker_s_mark_column_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_maker_s_mark_column_c4_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_maker_s_mark_column_c4_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_maker_s_mark_column_c4_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_maker_s_mark_column_c4_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_maker_s_mark_column_c4_", "-", if(validation_results[["a22_maker_s_mark_column_c4_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_maker_s_mark_column_c4_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_none_of_the_above_column_c7_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_none_of_the_above_column_c7_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_none_of_the_above_column_c7_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_none_of_the_above_column_c7_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_none_of_the_above_column_c7_", "-", if(validation_results[["a22_none_of_the_above_column_c7_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_none_of_the_above_column_c7_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_tito_s_column_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_tito_s_column_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_tito_s_column_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_tito_s_column_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_tito_s_column_", "-", if(validation_results[["a22_tito_s_column_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_tito_s_column_"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_tito_s_column_c1_ (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_tito_s_column_c1_"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_tito_s_column_c1_", rowCount = 10)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_tito_s_column_c1_", error = conditionMessage(e))
})

print(paste("Validated:", "a22_tito_s_column_c1_", "-", if(validation_results[["a22_tito_s_column_c1_"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_tito_s_column_c1_"]]$error)))
...
validation_results[["a14a_2"]] <- tryCatch({
  ...
  test_val <- sum(as.numeric(data[["A14a_2"]]) %in% c(1, 2), na.rm = TRUE)
  # Row 2: A14a_2 == 1
  if (!("A14a_2" %in% names(data))) stop("Variable 'A14a_2' not found")
  ...
  # Row 4: A14a_2 == 3,4
  if (!("A14a_2" %in% names(data))) stop("Variable 'A14a_2' not found")
  test_val <- sum(as.numeric(data[["A14a_2"]]) %in% c(3, 4), na.rm = TRUE)
  # Row 5: A14a_2 == 3
  if (!("A14a_2" %in% names(data))) stop("Variable 'A14a_2' not found")
  ...
  list(success = TRUE, tableId = "a14a_2", rowCount = 6)
