...
# Generated: 2026-02-07T04:09:40.394Z
# Tables to validate: 62
...
validation_results[["s1"]] <- tryCatch({
  ...
  # Row 1: NET - Any industry that would TERMINATE (NET)
...
# -----------------------------------------------------------------------------
# Table: s2_binned (frequency)
# -----------------------------------------------------------------------------

validation_results[["s2_binned"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "s2_binned", rowCount = 6)

}, error = function(e) {
  list(success = FALSE, tableId = "s2_binned", error = conditionMessage(e))
})

print(paste("Validated:", "s2_binned", "-", if(validation_results[["s2_binned"]]$success) "PASS" else paste("FAIL:", validation_results[["s2_binned"]]$error)))
...
validation_results[["s5"]] <- tryCatch({
  ...
  # Row 7: S5 == 5,6,7,8
  if (!("S5" %in% names(data))) stop("Variable 'S5' not found")
  test_val <- sum(as.numeric(data[["S5"]]) %in% c(5, 6, 7, 8), na.rm = TRUE)
  # Row 8: S5 == 5
  if (!("S5" %in% names(data))) stop("Variable 'S5' not found")
...
# -----------------------------------------------------------------------------
# Table: s8_dist_binned (frequency)
# -----------------------------------------------------------------------------

validation_results[["s8_dist_binned"]] <- tryCatch({
  ...
  # Row 4: S8 == 11-50
  if (!("S8" %in% names(data))) stop("Variable 'S8' not found")
  test_val <- sum(as.numeric(data[["S8"]]) >= 11 & as.numeric(data[["S8"]]) <= 50, na.rm = TRUE)

  list(success = TRUE, tableId = "s8_dist_binned", rowCount = 4)

}, error = function(e) {
  list(success = FALSE, tableId = "s8_dist_binned", error = conditionMessage(e))
})

print(paste("Validated:", "s8_dist_binned", "-", if(validation_results[["s8_dist_binned"]]$success) "PASS" else paste("FAIL:", validation_results[["s8_dist_binned"]]$error)))
...
validation_results[["a6"]] <- tryCatch({
  ...
  test_val <- sum(as.numeric(data[["A6"]]) %in% c(1, 2), na.rm = TRUE)
  # Row 2: A6 == 1
  if (!("A6" %in% names(data))) stop("Variable 'A6' not found")
  ...
  # Row 4: A6 == 3,4
  if (!("A6" %in% names(data))) stop("Variable 'A6' not found")
  test_val <- sum(as.numeric(data[["A6"]]) %in% c(3, 4), na.rm = TRUE)
  # Row 5: A6 == 3
  if (!("A6" %in% names(data))) stop("Variable 'A6' not found")
  ...
  list(success = TRUE, tableId = "a6", rowCount = 7)
...
validation_results[["a21"]] <- tryCatch({
  ...
  # Row 1: NET - Any of these brands (NET)
...
validation_results[["a22"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22", rowCount = 70)

}, error = function(e) {
  list(success = FALSE, tableId = "a22", error = conditionMessage(e))
})

print(paste("Validated:", "a22", "-", if(validation_results[["a22"]]$success) "PASS" else paste("FAIL:", validation_results[["a22"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_approachable (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_approachable"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_approachable", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_approachable", error = conditionMessage(e))
})

print(paste("Validated:", "a22_approachable", "-", if(validation_results[["a22_approachable"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_approachable"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_authentic (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_authentic"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_authentic", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_authentic", error = conditionMessage(e))
})

print(paste("Validated:", "a22_authentic", "-", if(validation_results[["a22_authentic"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_authentic"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_bold (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_bold"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_bold", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_bold", error = conditionMessage(e))
})

print(paste("Validated:", "a22_bold", "-", if(validation_results[["a22_bold"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_bold"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_down_to_earth (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_down_to_earth"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_down_to_earth", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_down_to_earth", error = conditionMessage(e))
})

print(paste("Validated:", "a22_down_to_earth", "-", if(validation_results[["a22_down_to_earth"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_down_to_earth"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_edgy (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_edgy"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_edgy", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_edgy", error = conditionMessage(e))
})

print(paste("Validated:", "a22_edgy", "-", if(validation_results[["a22_edgy"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_edgy"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_generous (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_generous"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_generous", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_generous", error = conditionMessage(e))
})

print(paste("Validated:", "a22_generous", "-", if(validation_results[["a22_generous"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_generous"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_independent (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_independent"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_independent", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_independent", error = conditionMessage(e))
})

print(paste("Validated:", "a22_independent", "-", if(validation_results[["a22_independent"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_independent"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_premium (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_premium"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_premium", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_premium", error = conditionMessage(e))
})

print(paste("Validated:", "a22_premium", "-", if(validation_results[["a22_premium"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_premium"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_reliable (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_reliable"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_reliable", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_reliable", error = conditionMessage(e))
})

print(paste("Validated:", "a22_reliable", "-", if(validation_results[["a22_reliable"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_reliable"]]$error)))

# -----------------------------------------------------------------------------
# Table: a22_sophisticated_refined (frequency)
# -----------------------------------------------------------------------------

validation_results[["a22_sophisticated_refined"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a22_sophisticated_refined", rowCount = 7)

}, error = function(e) {
  list(success = FALSE, tableId = "a22_sophisticated_refined", error = conditionMessage(e))
})

print(paste("Validated:", "a22_sophisticated_refined", "-", if(validation_results[["a22_sophisticated_refined"]]$success) "PASS" else paste("FAIL:", validation_results[["a22_sophisticated_refined"]]$error)))
...
validation_results[["a23_grey_goose_detail_keep_original"]] <- tryCatch({
  ...
  list(success = TRUE, tableId = "a23_grey_goose_detail_keep_original", rowCount = 5)

}, error = function(e) {
  list(success = FALSE, tableId = "a23_grey_goose_detail_keep_original", error = conditionMessage(e))
})

print(paste("Validated:", "a23_grey_goose_detail_keep_original", "-", if(validation_results[["a23_grey_goose_detail_keep_original"]]$success) "PASS" else paste("FAIL:", validation_results[["a23_grey_goose_detail_keep_original"]]$error)))
...
validation_results[["a7_1"]] <- tryCatch({
  ...
  # Row 1: NET - Any of these substances (NET)
