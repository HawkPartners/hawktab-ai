  ...
  try {
    ...
    if (validationResult.loopDetection?.hasLoops) {
      ...
      try {
        ...
        console.error('Loop handling failed (non-fatal):', loopError);
    ...
    if (surveyPath) {
      ...
      console.log('Step 1c: Processing survey document...');
      const surveyResult = await processSurvey(surveyPath, outputDir);
      surveyMarkdown = surveyResult.markdown;
      if (surveyMarkdown) {
        console.log(`Survey processed: ${surveyMarkdown.length} characters`);
      } else {
        console.warn(`Survey processing failed: ${surveyResult.warnings.join(', ')}`);
      }
    }
    ...
    console.log('Starting parallel paths...');
    ...
    console.log('Path B status initialized: running');
    ...
    if (surveyMarkdown) {
      ...
      console.log('Path C status initialized: running');
    } else {
      ...
      console.log('Path C skipped (no survey document)');
    }
    ...
    const pathBPromise = executePathB(verboseDataMap, spssPath, outputDir, updateParallelProgress, abortSignal)
      .then(async (result) => {
        ...
        console.log('Path B completed and result saved to disk');
        ...
        if (convexOrgId && convexProjectId) {
          try {
            ...
            console.log('Path B result uploaded to R2');
          } catch (r2Err) {
            console.warn('Failed to upload Path B result to R2 (non-fatal):', r2Err);
          }
        }
        ...
        try {
          ...
          console.warn('Failed to update pathBStatus in Convex:', err);
        }
      ...
      .catch(async (error) => {
        ...
        console.error(`Path B failed: ${errorMsg}`);
        ...
        try {
          ...
          console.warn('Failed to update pathBStatus error in Convex:', err);
        }
    ...
    const pathCPromise: Promise<PathCResult | null> = surveyMarkdown
      ? executePathC(surveyMarkdown, verboseDataMap, outputDir, abortSignal)
          .then(async (result) => {
            ...
            console.log('Path C completed and result saved to disk');
            ...
            if (convexOrgId && convexProjectId) {
              try {
                ...
                console.log('Path C result uploaded to R2');
              } catch (r2Err) {
                console.warn('Failed to upload Path C result to R2 (non-fatal):', r2Err);
              }
            }
            ...
            try {
              ...
              console.warn('Failed to update pathCStatus in Convex:', err);
            }
          ...
          .catch(async (error) => {
            ...
            console.warn(`Path C failed (non-fatal): ${errorMsg}`);
    ...
    if (wizardConfig?.bannerMode === 'auto_generate') {
      console.log('Auto-generate mode: using BannerGenerateAgent instead of Path A...');
      try {
        ...
        console.log(`BannerGenerateAgent: ${genResult.agent.length} groups generated (confidence: ${genResult.confidence})`);
        ...
        if (isAbortError(genError)) {
          console.log('Pipeline was cancelled during auto-generate');
          metricsCollector.unbindWideEvent();
        ...
        console.error(`Auto-generate failed: ${errorMsg}`);
        metricsCollector.unbindWideEvent();
      ...
      try {
        ...
        console.error(`Path A failed: ${errorMsg}`);
        metricsCollector.unbindWideEvent();
    ...
    console.log(`Path A completed in ${(pathADuration / 1000).toFixed(1)}s (Path B still running in background)`);
    ...
    console.log(`Path A: ${groupCount} groups, ${columnCount} columns, ${crosstabResult.result.bannerCuts.length} validated`);
    ...
    if (flaggedCrosstabColumns.length > 0) {
      console.log(`Review required: ${flaggedCrosstabColumns.length} columns need human review (CrosstabAgent mapping)`);
      console.log('Showing review UI immediately - Path B continues in background');
      ...
      console.log(`Review state saved to crosstab-review-state.json (wizardConfig included: ${wizardConfig !== undefined})`);
      ...
      if (convexOrgId && convexProjectId) {
        try {
          ...
          console.log('Review state files uploaded to R2');
        } catch (r2Err) {
    ...
    console.log('All CrosstabAgent mappings have high confidence - no review needed');
    console.log('Waiting for Path B and Path C to complete...');
    ...
    try {
      ...
      if (isAbortError(pathBError)) {
        console.log('Pipeline was cancelled during Path B');
        metricsCollector.unbindWideEvent();
    ...
    try {
      ...
      if (isAbortError(pathCError)) {
        console.log('Pipeline was cancelled during Path C');
        metricsCollector.unbindWideEvent();
    ...
    console.log(`All paths completed in ${(parallelDuration / 1000).toFixed(1)}s`);
    console.log(`Path B: ${tableAgentTables.length} table definitions`);
    if (pathCResult?.filterResult) {
      console.log(`Path C: ${pathCResult.filterCount} filters from ${pathCResult.skipLogicRuleCount} rules`);
    }
    ...
    console.log(`Converted to ${extendedTables.length} ExtendedTableDefinitions`);
    ...
    if (pathCResult?.filterResult && pathCResult.filterResult.translation.filters.length > 0) {
      ...
      console.log('Step 6: Applying skip logic filters...');
    ...
    console.log('Step 6b: GridAutoSplitter...');
    ...
    if (gridSplitResult.actions.length > 0) {
      console.log(`GridAutoSplitter: split ${gridSplitResult.summary.tablesSplit} tables (${gridSplitResult.summary.totalInput} → ${gridSplitResult.summary.totalOutput})`);
    ...
    if (surveyMarkdown) {
      console.log('Step 7: VerificationAgent (parallel, concurrency: 3)...');
      try {
        ...
        console.log(`VerificationAgent: ${verificationResult.tables.length} verified tables`);
      } catch (verifyError) {
        console.warn(`VerificationAgent failed — using unverified tables: ${verifyError instanceof Error ? verifyError.message : String(verifyError)}`);
        try {
      ...
      console.log('Step 7: No survey — skipping VerificationAgent (passthrough)');
    }
    ...
    console.log('Step 7b: TablePostProcessor...');

    const postPassResult = normalizePostPass(extendedTables);
    extendedTables = postPassResult.tables;
    console.log(`PostProcessor: ${postPassResult.stats.totalFixes} fixes, ${postPassResult.stats.totalWarnings} warnings`);
    ...
    console.log(`Screeners: ${sortingMetadata.screenerCount}, Main: ${sortingMetadata.mainCount}, Other: ${sortingMetadata.otherCount}`);

    // Stop after verification (wizard config)
    if (wizardConfig?.stopAfterVerification) {
      console.log('stopAfterVerification=true — completing with partial status');
      metricsCollector.unbindWideEvent();
    ...
    console.log('Step 8: Cut expression validation...');
    ...
    try {
      ...
      console.log(`Cut validation: ${cutReport.passed}/${cutReport.totalCuts} passed`);

      for (let retryAttempt = 1; retryAttempt <= MAX_CUT_RETRIES && cutReport.failed > 0; retryAttempt++) {
        console.log(`Cut retry attempt ${retryAttempt}/${MAX_CUT_RETRIES} for ${cutReport.failed} failed cuts...`);
        ...
        for (const failedGroupName of failedGroupNames) {
          ...
          if (!bannerGroup) {
            console.warn(`Skipping group "${failedGroupName}" — not found in agentBanner`);
            continue;
          }
          ...
          try {
            ...
            console.warn(`Cut retry failed for group ${failedGroupName}:`, retryErr);
          }
        }
        ...
        console.log(`Cut re-validation: ${cutReport.passed}/${cutReport.totalCuts} passed`);
      }

      if (cutReport.failed > 0) {
        console.warn(`${cutReport.failed} cuts still failing after retries — proceeding (R tryCatch safety net)`);
      }
    } catch (cutValidationError) {
      console.warn('Cut expression validation infrastructure failed (non-fatal):', cutValidationError);
    ...
    console.log('Step 8b: Validating R code per table...');
    ...
    if (loopTableCount > 0) {
      console.log(`Tagged ${loopTableCount} tables with loopDataFrame`);
    }
    ...
    console.log(`R Validation: ${rValidationReport.passedFirstTime} passed, ${rValidationReport.fixedAfterRetry} fixed, ${rValidationReport.excluded} excluded`);
    ...
    if (loopMappings.length > 0) {
      ...
      console.log('Step 8.5: LoopSemanticsPolicyAgent...');
      ...
      try {
        ...
        console.log(`LoopSemantics: ${loopSemanticsPolicy.bannerGroups.length} groups classified`);
      } catch (lspError) {
    ...
    console.log('Step 9: Generating R script...');
    ...
    if (staticValidationReport.invalidTables > 0 || staticValidationReport.warnings.length > 0) {
      ...
      console.log(`Static validation issues: ${staticValidationReport.invalidTables} invalid, ${staticValidationReport.warnings.length} warnings`);
    }

    console.log(`Generated R script (${Math.round(masterScript.length / 1024)} KB)`);
    console.log(`Tables in script: ${allTablesForR.length} (${validTables.length} valid, ${newlyExcluded.length} excluded)`);
    ...
    console.log('Step 10: Executing R script...');
    ...
    try {
      ...
      if (weightVariable && resultFiles.includes('tables-weighted.json') && resultFiles.includes('tables-unweighted.json')) {
        console.log('Found dual weighted/unweighted output');
        ...
        console.log('Step 11: Generating dual Excel workbooks...');
        ...
        try {
          ...
          console.log('Dual-output fmtOpts:', JSON.stringify(fmtOpts));
          // Weighted workbook
          ...
          excelGenerated = true;
          console.log('Generated dual Excel: crosstabs-weighted.xlsx + crosstabs-unweighted.xlsx');
        } catch (excelError) {
          console.error('Dual Excel generation failed:', excelError);
        }
        ...
        try {
          ...
          console.log('Generated data-streamlined.json');
        } catch (err) {
          console.warn('Could not generate streamlined data:', err);
        }
        ...
        console.log('Step 11: Generating Excel workbook...');
        ...
        try {
          ...
          console.log('Single-output fmtOpts:', JSON.stringify(fmtOpts));
          ...
          if (formatter.hasSecondWorkbook()) {
            ...
            console.log('Generated crosstabs-counts.xlsx');
          }
          excelGenerated = true;
          console.log('Generated crosstabs.xlsx');
        } catch (excelError) {
      ...
      if (errorMsg.includes('command not found')) {
        console.warn(`R not installed - script saved for manual execution`);
      } else {
        // Log full error (no truncation) for debugging
        console.error(`R execution failed:`, errorMsg);
    ...
    if (convexOrgId && convexProjectId) {
      try {
        // Extract timestamp from pipelineId (format: "pipeline-2026-02-14T02-00-07-829Z")
        ...
        r2Manifest = await uploadPipelineOutputs(
          ...
          outputDir,
          {
            projectName,  // Fetched earlier for ConsoleCapture
            runTimestamp,
          }
        );
