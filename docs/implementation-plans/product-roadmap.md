# Product Roadmap — Active Work Items

## 1. Automated Testing (Unit + Integration)

### Current State

We have 7 unit tests via Vitest, all focused on deterministic processors:
- `LoopDetector.test.ts`, `LoopCollapser.test.ts`, `LoopContextResolver.test.ts`, `FillRateValidator.test.ts` — loop/fill-rate pipeline
- `GridAutoSplitter.test.ts` — table splitting
- `transformStackedCuts.test.ts` — R expression transformation
- `retryWithPolicyHandling.test.ts` — retry logic

No integration tests. No API route tests. No Convex mutation tests. No end-to-end tests.

### What's Missing (Priority Order)

**Tier 1 — Deterministic logic tests (zero AI cost, fast, high value)**

These test the core processors that touch every pipeline run. A bug here breaks every dataset.

| Area | What to test | Files |
|------|-------------|-------|
| **WeightDetector** | Name gate rejects non-weight vars, accepts known patterns, scoring thresholds, structural suffix penalty | `src/lib/validation/WeightDetector.ts` |
| **DataMapProcessor** | Parent inference, type normalization, context enrichment. Feed known .sav metadata → assert enriched output | `src/lib/processors/DataMapProcessor.ts` |
| **TableGenerator** | Given a datamap + cuts spec → correct table structure, correct variable assignments, loop handling | `src/lib/tables/TableGenerator.ts` |
| **TablePostProcessor** | 7 deterministic rules (empty fields, section cleanup, base text, trivial NET removal, source ID casing, duplicate detection, orphan indent reset) | `src/lib/tables/TablePostProcessor.ts` |
| **FilterApplicator** | Given tables + filter rules → correct splitting, filter expression attachment | `src/lib/tables/FilterApplicator.ts` |
| **RScriptGeneratorV2** | Given tables.json → valid R script structure. Test expression interpolation, weight handling, stacked cuts, stat testing config | `src/lib/r/RScriptGeneratorV2.ts` |
| **sanitizeRExpression** | Injection prevention: test malicious inputs, edge cases, backtick escaping, special characters | `src/lib/r/sanitizeRExpression.ts` |
| **sortTables** | Table ordering: by section, by table ID, correct grouping | `src/lib/tables/sortTables.ts` |
| **ExcelFormatter** | Theme application, sheet structure, cell formatting. Can test with xlsx library assertions against generated buffer | `src/lib/excel/ExcelFormatter.ts` |

**Tier 2 — API route integration tests (zero AI cost, medium speed)**

These test the HTTP layer — auth, validation, rate limiting, error responses. Use Vitest with mocked Convex.

| Route | What to test |
|-------|-------------|
| `POST /api/validate-data` | Rejects non-.sav, enforces size limit, returns correct shape, handles R failure gracefully, weight candidates filtered correctly, loop summary computed |
| `POST /api/projects/launch` | Auth required, role check, duplicate project name → 409, file validation, rate limiting at critical tier, correct Convex mutations called |
| `GET /api/health` | Returns 200 without auth |
| `POST /api/runs/*/cancel` | Auth required, cancellation flag set, abort signal propagated |
| `POST /api/runs/*/review` | Auth required, review state transitions valid |

**Tier 3 — Pipeline integration tests (uses AI, expensive, slow — run sparingly)**

These are the "did the whole thing work" tests. Run against one small dataset with real AI calls.

| Test | What it validates |
|------|------------------|
| **Smoke test** | Single small dataset end-to-end → produces valid tables.json + R script + Excel. Assert: non-zero tables, valid R syntax, Excel has expected sheets |
| **Stacked data detection** | Upload Tito's dataset → assert isStacked or loop detection fires correctly |
| **Weight handling** | Upload weighted dataset → assert weight applied in R script |
| **Banner auto-generate** | No banner plan → assert AI generates banner, cuts extracted |

### Cost Analysis

- **Tier 1**: Free. Pure logic. Run on every commit. Target: < 10 seconds.
- **Tier 2**: Free. Mocked external calls. Run on every commit. Target: < 30 seconds.
- **Tier 3**: ~$2-5 per run (one small dataset). Run manually or on PR merge to main.

### Implementation Plan

1. Add test utilities: mock Convex client, mock R execution, fixture loader for .sav metadata snapshots
2. Write Tier 1 tests first — highest coverage per line of code, catches regressions in deterministic logic
3. Write Tier 2 tests — ensures API contracts don't break, auth/rate-limit enforcement holds
4. Write Tier 3 smoke test — single integration test against real pipeline
5. Add `npm test` to pre-commit hook or CI
