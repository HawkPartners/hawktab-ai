...
export async function sendPipelineNotification(params: PipelineNotificationParams): Promise<void> {
  try {
    ...
    // Build project URL
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://app.crosstabai.com';
    ...
    // Send via Resend
    const fromAddress = process.env.RESEND_FROM_ADDRESS || 'Crosstab AI <notifications@crosstabai.com>';
